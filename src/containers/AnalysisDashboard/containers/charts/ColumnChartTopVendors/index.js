...
const chartKey = "topvendorsquery"


const wrangler = (data) => {
    if (data) {
        let chartData = data
        let categories = chartData.map(item => item["supplier"])
...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        handleSelect: (e) => {
            dispatch(filterSelectVendor([e.point.category]))
        },
        handleDeselect: (e) => {
            dispatch(filterDeselectVendor(e))
        },
        fetchDataQuery: () => {
            dispatch(fetchDataQuery(chartKey, "/data/ds-12345/query/topvendors"))
        }
    }
}
